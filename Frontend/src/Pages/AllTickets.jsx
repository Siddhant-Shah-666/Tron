...
function AllTickets() {
  ...
  const [members, setMembers] = useState(null);
   ...
   const [formData, setFormData] = useState({
    ...
    const handleSubmit = async (e) => {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/projects/updateproject`, {
        method: "POST",
        credentials: "include",
        headers: {
  ...
  useEffect(()=>{
     try {
        fetch(`${import.meta.env.VITE_API_URL}/projects/project/getbyid/${projectId}`, {
          method: "GET",
          credentials: "include",
        })
          .then((res) => res.json())
  ...
  useEffect(() => {
    if (companyId && projectId) {
      ...
      try {
        fetch(`${import.meta.env.VITE_API_URL}/tickets/gettickets/bypro/${projectId}`, {
          method: "GET",
          credentials: "include",
        })
          .then((res) => res.json())
    ...
    <>
      ...
      <div className="w-full   md:w-[80vw] bg-blue-00 flex flex-col  justify-start md:items-center text-white">
        ...
        {projectId && (
          <div className="w-[70vw] md:w-[75vw]  flex flex-col justify-center items-start rounded font-bold p-3 gap-2 border bg-slate-950 border-cyan-400 shadow-lg shadow-cyan-400/30 text-cyan-400 ">
            ...
            <div className=" w-full ">
              <form action="" className="flex gap-3 flex flex-col" onSubmit={handleSubmit}>
                {!isAdmin && (<p className="text-xl mr-5">Developer : {project?.manager?.name}</p>)}
               ...
               <div className="manager">
                ...
                >
                  <option className="bg-slate-800" value="">{project?.manager?.name}</option>
                  {members &&
                    members.map((member) => (
                ...
                <div className="StatusAndPriority flex gap-3 ">
                ...
                >
                  <option className="bg-slate-800" value="">{project?.status}</option>
                  <option className="bg-slate-800" value="Open">Open</option>
                  <option className="bg-slate-800" value="Progress">Progress</option>
                ...
                <select
                  ...
                  className="border border-cyan-300 rounded-lg p-1 mb-2  w-[20vw] md:w-[10vw] "
                >
                  <option className="bg-slate-800" value="">{project?.priority}</option>
        ...
        <div className="stats w-full md:w-[77vw] h-auto md:h-[23vh] px-4 py-6 grid  grid-cols-2 md:grid-cols-4 gap-3 md:gap-5 bg-red-40">
          ...
          <StatsCard title="Close Tickets" stats={tickets?.filter(ticket=> ticket.status === "Closed").length} />
        </div>
